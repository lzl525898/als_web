      ...
      >
        <el-table-column
          ...
          :label="$t(`message.student_management_tableData_number`)"
           :width=numLableWidth
        ></el-table-column>
        <el-table-column align="center" prop="account" :width=userNumLableWidth :label="$t(`message.student_management_tableData_user_account`)"></el-table-column>
        <el-table-column align="center" prop="name"  :width=userNameLableWidth  :show-overflow-tooltip="true" :label="$t(`message.student_management_tableData_user_name`)"></el-table-column>
        <el-table-column align="center" :width=userSexLableWidth :label="$t(`message.student_management_tableData_user_gender`)">
        ...
        <el-table-column align="center" prop="birth" :width=userBirthLableWidth  :label="$t(`message.student_management_tableData_user_borth`)"></el-table-column>
        <el-table-column align="center" prop="phone" :width=userPhoneLableWidth :label="$t(`message.student_management_tableData_parent_phone`)"></el-table-column>
        <el-table-column align="center" prop="classes.className" :min-width="userclassNameLableWidth"  :label="$t(`message.student_management_tableData_user_class`)"></el-table-column>
        <el-table-column align="center" prop="school" :min-width=userschoolNameLableWidth :label="$t(`message.student_management_tableData_user_school`)"></el-table-column>
        <el-table-column  v-if="judgeCnEn=='en'" align="center" :label="$t(`message.student_management_tableData_user_operation`)" fixed="right" width="480">
          <template slot-scope="scope">
            <!--            <el-button-->
        ...
        <el-table-column v-else align="center"  :label="$t(`message.student_management_tableData_user_operation`)" fixed="right" width="380">
             <template slot-scope="scope">
                <!--            <el-button-->
                <!--              v-if="isShowLearnReport"-->
                <!--              size="mini"-->
      ...
      <el-row>
        <el-col :span="16" :offset="4">
          <el-form
            ...
            :label-width=this.editLabelWidth
            class="demo-ruleForm"
          >
            ...
            <el-form-item v-if="judgeCnEn=='en'" :label="$t(`message.student_management_studentEditDialogVisible_student_sex`)" prop="sex">
              <el-radio-group v-model="ruleFormWithStudent.sex">
                ...
                <el-radio style="margin-top:8px" :label="0">{{$t("message.student_management_tableData_user_sex_secrecy")}}</el-radio>
              </el-radio-group>
            </el-form-item>
             <el-form-item v-else :label="$t(`message.student_management_studentEditDialogVisible_student_sex`)" prop="sex">
              <el-radio-group v-model="ruleFormWithStudent.sex">
                <el-radio :label="1">{{$t("message.student_management_tableData_user_sex_n")}}</el-radio>
                <el-radio :label="2">{{$t("message.student_management_tableData_user_sex_nv")}}</el-radio>
...
<!--        确定-->
    ...
    <el-dialog :title="$t(`message.student_management_tableData_operation_password`)" :visible.sync="pwdDialogVisible" destroy-on-close width="20%">
      <el-row>
        <el-col :span="4">
...
export default {
  ...
  data() {
    return {
      ...
      userPhoneLableWidth: "",
      userSexLableWidth: "",
      userclassNameLableWidth: "",
      userschoolNameLableWidth: ""
    };
  },
  created() {
  ...
  watch: {
      "$i18n.locale": function() {
          this.addStudentData = [
              {
                  id: "all",
                  label: this.$t(`message.student_all_student`),
                  children: [
                      {
                          id: "other",
                          label: this.$t(`message.student_all_class_student`),
                          children: []
                      }
                  ]
              }
          ]
          this.rulesWithStudent.name[0].message = this.$t(`message.student_management_enter_student_name`)
          this.rulesWithStudent.phone[0].message = this.$t(`message.student_management_studentEditDialogVisible_input_phone`)
          this.rulesWithStudent.phone[1].message = this.$t(`message.student_management_studentEditDialogVisible_input_phone_content`)
          this.judgeCnEn = storageUtil.getLang();
          if (this.judgeCnEn == "zh" || this.judgeCnEn == "cht") {
              this.editLabelWidth = "100px";
              this.numLableWidth = "60px";
              this.userNumLableWidth = "120px";
              this.userNameLableWidth = "120px";
              this.userBirthLableWidth = "100px";
              this.userPhoneLableWidth = "105px";
              this.userSexLableWidth = "70px";
              this.userclassNameLableWidth = "150px";
              this.userschoolNameLableWidth = "150px";
          } else {
              this.numLableWidth = "120px";
              this.editLabelWidth = "135px";
              this.userNumLableWidth = "135px";
              this.userNameLableWidth = "120px";
              this.userBirthLableWidth = "135px";
              this.userPhoneLableWidth = "140px";
              this.userSexLableWidth = "175px";
              this.userclassNameLableWidth = "150px";
              this.userschoolNameLableWidth = "150px";
          }
      },
  ...
  mounted() {
    ...
    if (
      ...
      this.$store.state.systemMenu != "" &&
      this.$store.state.systemMenu.length >= 0
    ) {
      this.isShowLearnReport =
        this.$store.state.systemMenu.findIndex(
          item => item.url == "studyMan"
        ) >= 0
          ? true
          : false;
    }
  ...
  methods: {
    ...
    bindSuperblockly(obj) {
      ...
      superBlockly.bindAccount(
        "stu",
        this,
        accountInfo,
        promptUtil.loading(this)
      );
    },
    // 点选转化学生的操作相应
      ...
      getAllClass(
        ...
        .then(res => {
          ...
          if (res.code == SUCCESS_CODE) {
            ...
            //   }
            // ];
            this.addStudentData[0].children[0].children = children
            getConsultingManagement(
              qs.stringify({
    ...
    genErrorListHtml(conArray) {
      ...
      conArray.forEach((item, index) => {
        const content =
          ...
          item.data.phone +
          ")" +
          this.$t(`message.student_html_error`) +
          item.msg;
        setTimeout(() => {
          this.$notify.error({
            title: this.$t(`message.dialog_header_title_prompt`),
            message: content,
            duration: 0
          });
        }, offsetTime * index);
    ...
    handleAddStuFromCon() {
      if (this.genStudentIdsFromCon().length == 0) {
        promptUtil.warning(this, this.$t(`message.student_convert_student`));
        return;
      }
      ...
      conConsultingManagement(
        ...
        .then(res => {
          if (res.code == SUCCESS_CODE) {
            ...
            if (successNum == 0) {
              //
              promptUtil.error(this, this.$t(`message.student_convert_failed`));
            } else {
              promptUtil.success(this, this.$t(`message.student_convert_success`) + successNum + this.$t(`message.student_convert_success_student`));
            }
            const loading = promptUtil.loading(this);
            this.getAllStudentList(loading);
          } else {
    ...
    handleAddStuFromNew() {
      if (this.genStudentNamesFromTextArea() == "") {
        promptUtil.warning(this, this.$t(`message.student_add_student_with_name`));
        return;
      }
      let valdate = /[,，]/g;
      if (valdate.test(this.addStudentFromTextarea.trim())) {
        promptUtil.warning(this, this.$t(`message.student_add_student_valid_names`));
        return;
      }
      this.isLoadingWithAdd = true;
      addStudent(
    ...
    showEditStuDialog(obj) {
      ...
      this.ruleFormWithStudent.birth =
        obj.row.birth == null || obj.row.birth == ""
          ? ""
          : moment(obj.row.birth).toDate();
      this.ruleFormWithStudent.classes = obj.row.classes.className;
      this.ruleFormWithStudent.sex = obj.row.sex*1;
    ...
    showPwdStuDialog(obj) {
      ...
      this.pwdDialogContent.account = this.$t(`message.student_management_student_account`) + " : " + obj.row.account;
      this.pwdDialogContent.nick = this.$t(`message.consult_conversion_student_name`) + " : " + obj.row.name;
    },
    // 显示删除学生对话框
    showDelStuDialog(obj) {
      this.currentResetStuId = obj.row.id;
      this.delDialogVisible = true;
      this.delDialogContent = this.$t(`message.consult_dialog_content_del`) + " 【 " + obj.row.name + " 】 ？";
    ...
    handleEditStudent() {
      if (
        ...
        promptUtil.warning(this, this.$t(`message.student_management_input_student_name`));
        return;
      }
      if (
        ...
        promptUtil.warning(this, this.$t(`message.student_management_input_parent_phone`));
        return;
      }
      // else {
        ...
        .then(res => {
          ...
          if (res.code == SUCCESS_CODE) {
            ...
            this.queryFromServer[
              this.ruleFormWithStudent.current +
                (this.currentPage - 1) * this.pageSize
            ].name = this.ruleFormWithStudent.name.trim();
            this.queryFromServer[
              this.ruleFormWithStudent.current +
                (this.currentPage - 1) * this.pageSize
            ].phone =
            ...
            this.queryFromServer[
              ...
              formatBirth === "Invalid date" ? "" : formatBirth;
            this.queryFromServer[
              this.ruleFormWithStudent.current +
                (this.currentPage - 1) * this.pageSize
    ...
    exportStudent() {
      if (this.queryFromServer.length == 0) {
        promptUtil.warning(this, this.$t(`message.student_management_no_student_info`));
      } else {
        const targetParams = [];
        const params = { head: [], body: [], name: "" };
        params.head = [
          this.$t(`message.student_management_label_account`),
          this.$t(`message.teacher_form_teacher_default_pwd`),
          this.$t(`message.consult_conversion_student_name`),
          this.$t(`message.student_management_studentEditDialogVisible_student_sex`),
          this.$t(`message.student_management_tableData_user_borth`),
          this.$t(`message.student_management_tableData_parent_phone`),
          this.$t(`message.student_management_tableData_user_class`),
          this.$t(`message.student_management_tableData_user_school`)
        ];
        params.name = storageUtil.readTeacherInfo().school_name + "—" + this.$t(`message.student_management_student_data`);
        this.queryFromServer.map(item => {
          ...
          if (item.sex == 1) {
            //
            tmpArr.push(this.$t(`message.string_label_male`));
          } else if (item.sex == 2) {
            tmpArr.push(this.$t(`message.string_label_female`));
          } else {
            tmpArr.push(this.$t(`message.student_management_tableData_user_sex_secrecy`));
          }
  ...
  computed: {
    ...
    currentClassName: {
      get() {
        const content = this.$t(`message.student_management_create_school_student_current_class`) + " : ";
