    ...
    <div>
      ...
      <el-card shadow="always" style="margin-top: 20px;">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px">
          ...
          <el-form-item>
            <el-button type="primary" @click="onCreateSubmit" :loading="isSaveLoading" :disabled="pictureUploadStatus==1">保存</el-button>
  ...
  export default {
    ...
    methods:{
      initData(){
        ...
        findCustomSelfById(qs.stringify({
          school_id: storageUtil.readTeacherInfo().school_id,
          course_id: this.$route.params.id
        })).then(res=>{
          if(res.code==SUCCESS_CODE){
            this.analysisFromServer(res.data)
          }else{
            this.$router.push({path:ROUTER_CUSTOM})
          }
          loading.close()
          ...
          loading.close()
          promptUtil.LOG("findCustomSelfById-err",err)
        })
      },
      analysisFromServer(data){
        this.ruleForm.picture = data.class_banner
        ...
        this.categoryArray = data.category_all
        this.ruleForm.category = stringUtil.string2IntArray(data.category,",")
      },
      onCreateSubmit(){
        this.$refs['ruleForm'].validate((valid) => {
          if (valid) {
            ...
            addCustomSelf(qs.stringify({
              ...
              course_id: this.$route.params.id, // 添加 0 编辑 id
            })).then(res=>{
              if(res.code==SUCCESS_CODE){
                this.$router.push({path: ROUTER_CUSTOM})
              }else{
              ...
              this.isSaveLoading = false
                promptUtil.LOG("addCustomSelf-err",err)
            })
          } else {
