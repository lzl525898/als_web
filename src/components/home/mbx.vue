      ...
      <el-col :span="12">
        <el-card class="box-card" shadow="always">
          <div slot="header" class="clearfix" style="display: flex;align-items: center">
            ...
            <div style="display: flex;justify-content: flex-end;flex: 1">
              <div style="line-height:30px;margin-right: 10px;font-size: 16px">{{showTime}}</div>
              <div class="block">
                <el-date-picker
                  style="width: 160px;"
                  size="small"
          ...
          <div id="signInStatus" style="height:360px !important;width: 100% !important;margin-top:10px;"></div>
        </el-card>
      </el-col>
      <el-col :span="12">
        <el-card class="box-card" shadow="always">
          ...
          <el-row>
            <el-col :span="12">
              ...
              <div id="classStatus" style="height:360px !important;width: 100% !important;margin-top:10px;"></div>
              <!--              </div>-->
            </el-col>
            <el-col :span="12">
              <!--              <div style="width:200px">-->
              <div id="classStudentStatus" style="height:360px !important;width: 100% !important;margin-top:10px;"></div>
              <!--              </div>-->
            </el-col>
          </el-row>
        </el-card>
...
<script>
    ...
    export default {
          ...
          '$i18n.locale': function () {
            this.classStatus = {
              ...
              legend: {
                orient: 'vertical',
                x: 'left',
                data: [this.$t('message.string_label_course_temporary'), this.$t('message.string_label_course_cycle')],
              },
              series: [
                {
                  ...
                  data: [
                    {value: this.temporary, name:this.$t('message.string_label_course_temporary')},
                    {value: this.cycle, name:this.$t('message.string_label_course_cycle')},
                  ]
                },
                {
            ...
            this.classStudentStatus = {
              ...
              legend: {
                orient: 'vertical',
                x: 'left',
                data: [this.$t('message.index_course_number_temporary_number'), this.$t('message.index_course_number_cycle_number')],
              },
              series: [
                {
                  ...
                  data: [
                    {value: this.temporaryNum, name: this.$t('message.index_course_number_temporary_number')},
                    {value: this.cycleNum, name: this.$t('message.index_course_number_cycle_number')},
                  ]
                },
                {
            ...
            this.classStatisticsOption = {
              ...
              legend: {
                data: [this.$t('message.string_label_class_attendance'), this.$t('message.string_label_class_absence'), this.$t('message.string_label_class_takeleave'),]
              },
              grid: {
                left: '3%',
                right: '4%',
        ...
        methods: {
            ...
            //初始化Chart
            initChart() {
                this.classStatusChart = echarts.init(document.getElementById('classStatus'));
                this.classStudentStatusChart = echarts.init(document.getElementById('classStudentStatus'));
                this.signInStatusChart = echarts.init(document.getElementById('signInStatus'));
            },
            //课时内容统计  临时课 周期课
            initClassStatus() {
            ...
            get_week_time(time, type) {//time 目标日期 格式日期或时间戳 type 返回格式 date日期 time 日期时间 timestamp(默认)毫秒时间戳
                ...
                if (type == "date") {
                    return [this.get_daystart_date(ds), this.get_daystart_date(ds + 60 * 60 * 24 * 1000 * 7 - 1)];
                } else if (type == "time") {
                    return [this.get_daystart_date(ds, "time"), this.get_daystart_date(ds + 60 * 60 * 24 * 1000 * 7 - 1, "time")];
                } else {
                    return [new Date(ds).getTime(), new Date(ds + 60 * 60 * 24 * 1000 * 7).getTime()];
                }
            },
            get_daystart_date(time, type) {//时间戳转日期函数
                if (!type) type = "date";
                var Y, M, D, h, m, s;
            ...
            systematicSelectTime(val) {
                if (val) {
                    ...
                    this.systematicStartTime = moment(Math.round(val[0].getTime())).format('YYYY-MM-DD')
                    this.systematicEndTime = moment(Math.round(val[1].getTime())).format('YYYY-MM-DD')
                } else {
            ...
            initGetSystemInformation() {
                //获取系统统计数据
                getSystematicStatistics(qs.stringify({
                    user_id: storageUtil.readTeacherInfo().id,
            ...
            initGetBeginClassInformation() {
                getBeginClassStatistics(qs.stringify({
                    user_id: storageUtil.readTeacherInfo().id,
            ...
            initGetclassContentInformation() {
                getclassHourStatistics(qs.stringify({
                    user_id: storageUtil.readTeacherInfo().id,
